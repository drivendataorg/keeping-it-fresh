'''
This file takes the data provided by Yelp and the contest hosts and builds a training data set used in LearnTest.py for building models
Date: August 28, 2015
'''
...
#nevertheless this code is left in for completeness
for key in businesses.keys():
    ...
    for i in range(0, len(business.inspections)):
        ...
        if i == 0 and len(business.inspections) > 1 and business.inspections[i+1].days - business.inspections[i].days <= 40:
        ...
        if i > 0 and len(business.inspections) > i+1 and business.inspections[i].days - business.inspections[i-1].days > 40 and business.inspections[i+1].days - business.inspections[i].days <= 40:
    ...
    for i in range(1, len(business.inspections)):
        if business.inspections[i-1].is_first_in_set and business.inspections[i].days - business.inspections[i-1].days <= 40:
            business.inspections[i].is_second_in_set = True
            business.inspections[i].related_inspection = "," + business.inspections[i-1].one_star + "," + business.inspections[i-1].two_star + "," + business.inspections[i-1].three_star

    # see if inspection is third within a set
    for i in range(1, len(business.inspections)):
        if business.inspections[i-1].is_second_in_set and business.inspections[i].days - business.inspections[i-1].days <= 40:
            business.inspections[i].is_later_in_set = True

    # see if inspection is fourth or further within a set
    for i in range(1, len(business.inspections)):
        if not (business.inspections[i-1].is_second_in_set or business.inspections[i-1].is_first_in_set) and business.inspections[i].days - business.inspections[i-1].days <= 40:
            business.inspections[i].is_later_in_set = True
...
#build the training data set by merging the necessary variables onto each inspection in the training set
part_train = []
for key in businesses:
    num_inspections = 0
    num_violations = 0
    ...
    num_three_viol = 0
    for i in businesses[key].inspections:
        #wow, I think I made the mistake of not removing 2012-12-30 and 2013-12-30 inspections from the training data when I should have...
        #the line below was probably mistakenly commented out and forgotten to uncomment
        #if i.inspection_id and i.date != "2012-12-30" and i.date != "2013-12-30":
        num_inspections += 1
        ...
        #transform violations from the one-, two-, three-star format to a single violation variable
        violations = int(i.one_star) + 2*int(i.two_star) + 5*int(i.three_star)
        ...
        part_train.append(info)

#write out the file
f = codecs.open("train_data_final.csv","w",'utf-8')
...
#write the rows of training data. Note that the first row is written twice, this is because I'm not fluent with Pandas and was somehow losing the first row during training so I added an extra row to account for that
f.write(part_train[0] + "\n")
for line in part_train:
    f.write(line)
    f.write("\n")
f.close()
